function [s,h]=grpwdiffwavgraph4lines(wav1,group,wav2,samprate,resamprate,alpha,outliers,printts,patches,patchlen,xax,showlegend,pscalemag)
...
if samprate==resamprate
  for t=1:wavlen
    mr=mhreg(wav1(:,t),group,wav2(:,t),outliers);
    s(t)=mr.dr.p;
  end
else
  for t=1:samptoavg:wavlen
    lt=min(wavlen,t+samptoavg-1);
    mr=mhreg(mean(wav1(:,t:lt)')',group,mean(wav2(:,t:lt)')'); 
    s(t:lt)=mr.dr.p;
...
%legend('Cond1 Group1','Cond2 Group1','Cond1 Group2','Cond2 Group2') 
